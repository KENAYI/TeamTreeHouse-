# Binary Search implements a quicker searching algorithm than linear search
...
# Function implementing binary search on names
def bBinary_search(collection, target):
    first = 0
    last = len(collection) - 1
    # stopping condition
    while first <= last:
        # find index at midpoint
        midp = (first + last) // 2
        # incase midp is infact our target value
        if collection[midp] == target:
            return midp
        elif collection[midp] < target:
            first = midp + 1
        else: 
            last = midp - 1
    return None

# type sorten_names.txt into console as argv
for n in search_names:
    index = bBinary_search(names, n)
    print(index)